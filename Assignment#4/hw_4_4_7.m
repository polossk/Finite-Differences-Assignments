...
%%%
%% Numerical Solution(Correct Scheme, Comparing with HW 4.4.6)
%%%
ts = [0.06, 0.1, 0.9];
dx = 0.05; dy = 0.05; dt = 0.01;
[u, X, Y] = solver_4_4_7(dx, dy, dt);
plotSlice_4_4_3(ts, dt, u, X, Y);

%% Code: Solver of Functionn
%%%
%
%   function [u, X_, Y_] = solver_4_4_7( dx, dy, dt )
...
%       coef_prdx2_times_inv_mrdx2 = coef_prdx2 * coef_inv_mrdx2;
%   
%       for kk = 1 : N
%           u__ = u(:, :, kk);
%           u_ = u__(2 : end - 1, 2 : end - 1);
%           coef_Right_Hand = coef_prdy2 * u_ * coef_prdx2_times_inv_mrdx2;
